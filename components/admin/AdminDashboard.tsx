...
function DistributionList({
  ...
  if (items.length === 0) {
    return <p className="text-sm text-ink-muted">Sin datos todavía.</p>;
  }
  return (
    <ul className="flex flex-col gap-2">
      {items.map((item) => (
        <li key={item.label} className="flex items-center gap-3 text-sm">
          <span className="w-44 shrink-0 truncate text-ink-soft">{item.label}</span>
          <span className="h-2 flex-1 overflow-hidden rounded-full bg-blush-100">
            <span
              className="block h-full rounded-full bg-brand-600"
              style={{ width: `${(item.count / max) * 100}%` }}
            />
          </span>
          <span className="w-8 shrink-0 text-right text-ink-muted">{item.count}</span>
        </li>
      ))}
    </ul>
  );
...
export function AdminDashboard() {
  ...
  return (
    <PageShell>
      ...
      {loading && <p className="text-ink-muted">Cargando métricas…</p>}

      {!loading && problem && (
        <div className="mb-8 rounded-2xl border border-line bg-white/60 p-4 text-sm text-ink-soft">
          <p className="mb-2">{problem}</p>
          <p className="text-ink-muted">
            Comprueba que ejecutaste la migración SQL y que registraste tu correo con{" "}
            <code className="rounded bg-blush-100 px-1">
      ...
      <section className="mb-8">
        ...
        <p className="mb-3 text-sm text-ink-muted">
          Versión activa: <code className="rounded bg-blush-100 px-1">{ALGORITHM_VERSION}</code>.
          Los pesos se editan en{" "}
          <code className="rounded bg-blush-100 px-1">data/classification-rules.ts</code> y
      ...
      <section>
        ...
        <p className="mb-3 text-sm text-ink-muted">
          ...
          <code className="rounded bg-blush-100 px-1">data/seasons.ts</code> y{" "}
          <code className="rounded bg-blush-100 px-1">data/palettes.ts</code>.
        </p>
        <ul className="flex flex-col gap-2">
